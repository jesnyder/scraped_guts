...
def targeted_word_count():
    ...
    if  5 in tasks: plot_annual_count_2()
    #if  5 in tasks: bar_annual_count()
    ...
    for name_article in retrieve_list('type_article'):
        ...
        for file in os.listdir(retrieve_path('term_compare')):
            ...
            for term in term_list:
                ...
                term_cdf = str(term + '_cdf')
                ref = list(df[term_cdf])
                ref_num = ref[-1]

                print('df.columns = ')
                print(df.columns)
                print('term = ' + term)
                col_str = str(term + '_value')
                print('col_str = ' + col_str)
                ref_value = round(sum(list(df[col_str]))/1000000,1)

                #label_term = str(ref_num) + ' ' + str(round(100*yy[-1],2)) + '% ' + term
                label_term = str(ref_num) + ' grants funded for $' + str(ref_value)  + ' million '+ term
...
def bar_annual_count():
    ...
    for name_article in retrieve_list('type_article'):
        ...
        for file in os.listdir(retrieve_path('term_compare')):
            ...
            plt.close('all')
            figure, axes = plt.subplots()
            plot_row, plot_col, plot_num = 1, 1, 0
            plt.figure(figsize=(plot_col*retrieve_format('fig_wid'), plot_row*retrieve_format('fig_hei')))
            plt.rc('font', size=retrieve_format('plot_font_size'))
